...
class JenaCmd(object):
    ...
    def load_nt(tdb_path, nt_path, jena_path="/home1/peng/project/apache-jena-3.7.0/bin"):
        status, out = subprocess.getstatusoutput('sh {}/tdbloader --loc="{}" "{}"'.format(jena_path, tdb_path, nt_path))
        print(out)
        return status

def seg_nt(source_nt, max_len=5000000.0):
    out_file_list = []
    _, total_line = subprocess.getstatusoutput("wc -l {}".format(source_nt))
    ...
    for i in tqdm(range(int(math.ceil(float(total_line)/max_len)))):
        ...
        status, _ = subprocess.getstatusoutput("sed -n '{},{}p' {} > {}".format(int(max_len*(i)+1), int(max_len*(i+1)), source_nt, ith_file_name))
        if status == 1:
